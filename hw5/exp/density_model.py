...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        counts = np.zeros(shape=[bsize], dtype=int)
...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.get_variable('log_std', shape=[z_size], dtype=tf.float32, initializer=tf.constant_initializer(0), trainable=True)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood, feed_dict={
            self.state1: state1,
            self.state2: state2,
            self.discrim_target: np.ones((state1.shape[0], 1), dtype=np.float32)
        })
        return likelihood
